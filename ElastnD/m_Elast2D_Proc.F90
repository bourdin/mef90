...
! UNCOMMENT THE FOLLOWING LINES TO USE MULTIPLE UNSYNC I/O
...
! UNCOMMENT THE FOLLOWING TO BCAST THE INPUT FILE
!!$    If (MEF90_MyRank ==0) Then
!!$       Write(CharBuffer, 100) 'Simulation name: '
...
!!$    End If
!!$
!!$    Call PetscGetTime(BCastTS, iErr)
!!$    Call Broadcast_Geom(Geom, 0)
!!$#ifdef MEF90_TIMING
!!$    Call PetscGetTime(BCastTF, iErr)
!!$    If (MEF90_MyRank == 0) Then
!!$       Write(CharBuffer, *) 'Time in Broadcast_Geom:                  ',      &
!!$            & BCastTF - BCastTS, '\n'c
!!$       Call PetscPrintf(PETSC_COMM_WORLD, CharBuffer, iErr)
!!$    End If
!!$#endif
!!$
!!$    Call PetscGetTime(BCastTS, iErr)
!!$    Call Broadcast_Rupt_Params(Params, 0)
!!$#ifdef MEF90_TIMING
!!$    Call PetscGetTime(BCastTF, iErr)
!!$    If (MEF90_MyRank == 0) Then
!!$       Write(CharBuffer, *) 'Time in Broadcast_Rupt_Param:            ',      &
!!$            & BCastTF - BCastTS, '\n'c
!!$       Call PetscPrintf(PETSC_COMM_WORLD, CharBuffer, iErr)
!!$    End If
!!$#endif
!!$
!!$    Call PetscGetTime(BCastTS, iErr)
!!$    Call Broadcast_Node(Node_db, 0)
!!$#ifdef MEF90_TIMING
!!$    Call PetscGetTime(BCastTF, iErr)
!!$    If (MEF90_MyRank == 0) Then
!!$       Write(CharBuffer, *) 'Time in Broadcast_Node:                  ',      &
!!$            & BCastTF - BCastTS, '\n'c
!!$       Call PetscPrintf(PETSC_COMM_WORLD, CharBuffer, iErr)
!!$    End If
!!$#endif
...
!!$
!!$    Call PetscGetTime(BCastTS, iErr)
!!$    Call Broadcast_Element(Elem_db, Geom, 0)
!!$#ifdef MEF90_TIMING
!!$    Call PetscGetTime(BCastTF, iErr)
!!$    If (MEF90_MyRank == 0) Then
!!$       Write(CharBuffer, *) 'Time in Broadcast_Element:               ',      &
!!$            & BCastTF - BCastTS, '\n'c
!!$       Call PetscPrintf(PETSC_COMM_WORLD, CharBuffer, iErr)
!!$    End If
!!$#endif
...
!!$
!!$    Allocate(Params%BC_Type_X(Geom%Num_Elem_Blks))
!!$    Allocate(Params%BC_Type_Y(Geom%Num_Elem_Blks))
!!$    Allocate(Params%BC_Type_Z(Geom%Num_Elem_Blks))
...
!!$!    Write(100+MEF90_MyRank, *) Node_db
!!$       Write(200+MEF90_MyRank, *) '================', iE, '================'
!!$       Write(200+MEF90_MyRank, *) Elem_db(iE)%Nb_DoF
!!$       Write(200+MEF90_MyRank, *) Elem_db(iE)%NB_Gauss
!!$       Write(200+MEF90_MyRank, *) Elem_db(iE)%ID_EL
!!$       Write(200+MEF90_MyRank, *) Elem_db(iE)%ID_DoF
!!$       Write(200+MEF90_MyRank, *) Elem_db(iE)%BF
!!$       Write(200+MEF90_MyRank, *) Elem_db(iE)%GradS_BF
!!$       Write(200+MEF90_MyRank, *) Elem_db(iE)%Gauss_C
!!$    End Do
!!$!    Write(300+MEF90_MyRank, *) Geom
!!$!    Write(400+MEF90_MyRank, *) Params
    ...
    Call MatCreateMPIAIJ(PETSC_COMM_WORLD, MySD%Num_Nodes, MySD%Num_Nodes,    &
         & Geom%Num_Nodes * Geom%Num_Dim, Geom%Num_Nodes * Geom%Num_Dim, 60,  &
...
111 Format('Stress%XZ Min / Max:    ', 2(ES10.3,' '))
  ...
  Subroutine Assemb_Mat_Elast(MR, Geom, Params, Elem_db, Node_db)
    ...
    Do_iBlk: Do iBlk = 1, Geom%Num_Elem_Blks
       ...
       Do_iE: Do iELoc = 1, Geom%Elem_Blk(iBlk)%Num_Elems
          ...
          Do_iSLSig: Do iSLSig = 1, Nb_DoF
             ...
             Do iG = 1, Nb_Gauss
                Sigma(iG) = 2.0_Kr * Mu * Elem_db(iE)%GradS_BF(iSLSig,iG)
                Sigma(iG)%XX = Sigma(iG)%XX +&
                     & Lambda * Trace(Elem_db(iE)%GradS_BF(iSLSig,iG))
    ...
    Call AOApplicationToPETSc(MySD%EXO_AO, Geom%Num_Nodes * Geom%Num_Dim,    &
         & EXO_Indices, iErr)
  ...
  Subroutine Assemb_RHS_Elast(RHS, Geom, Params, Elem_db, Node_db, BC_Loc,    &
       & F_Inc)
    ...
    Do_iBlk: Do iBlk = 1, Geom%Num_elem_blks
       If (.NOT. Params%Has_Force(iBlk)) Then
          CYCLE
       End If
       Force%X = Params%Force(iBlk)%X * F_Inc
       Force%Y = Params%Force(iBlk)%Y * F_Inc
#ifdef PB_3D
       Force%Z = Params%Force(iBlk)%Z * F_Inc
#endif
       Do_iE: Do iELoc = 1, Geom%Elem_blk(iBlk)%Num_Elems
          iE = Geom%Elem_blk(iBlk)%Elem_ID(iELoc)
          If (.NOT. MySD%IsLocal_Elem(iE)) Then
             ...
             Tmp_Val = 0.0_Kr
                Do_iG: Do iG = 1, Nb_Gauss
                   Tmp_Val = Tmp_Val + Elem_db(iE)%Gauss_C(iG) *              &
                        & ( Force .DotP. Elem_db(iE)%BF(iSL1,iG) )
                End Do Do_iG
    ...
    Call AOApplicationToPETSc(MySD%Loc_AO, Geom%Num_Nodes * Geom%Num_Dim,    &
         & Loc_Indices, iErr)
...
!!$            & X, INSERT_VALUES, iErr)
  ...
  Subroutine Calc_Ener(DISP_Loc, Geom, Params, Elem_db, Node_db, SD, F_Inc,   &
       & Ener) 
          ...
          Do_iSLSig: Do iSLSig = 1, Nb_DoF
             ...
             If (Params%Has_Force(iBlk)) Then
                Do iG = 1, Nb_Gauss
                   MyEner = MyEner - Elem_db(iE)%Gauss_C(iG) *                &
                        & DISP_Ptr(Loc_Indices(iSGSig)+1) *                   &
                        & (Force .DotP. Elem_db(iE)%BF(iSLSig, iG))
                End Do
             End If
          End Do Do_iSLSig
          DeAllocate(Sigma)
