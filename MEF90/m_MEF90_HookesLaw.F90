...
!!!
   subroutine MEF90GetHookesLaw(comm, prefix, dim, HookesLaw, ierr)
      ...
      HookesLawType = MEF90HookesLawEnumIsotropic 
      PetscCall(PetscOptionsBegin(comm, trim(prefix), "Options for MEF90HookesLaw", "MEF90", ierr))
         PetscCall(PetscOptionsEnum("-HookesLaw_type", "Hookes law type", "MEF90", MEF90HookesLawEnumList, MEF90HookesLawEnumIsotropic, HookesLawType, PETSC_NULL_BOOL, ierr))
      PetscCall(PetscOptionsEnd(ierr))

      select case (HookesLawType)
         case (MEF90HookesLawEnumIsotropic)
            select case(dim)
               case(2)
...
!       class(MEF90HookesLaw), intent(in) :: A, B
!       class(MEF90HookesLaw), allocatable :: MEF90HookesLawSum

!       type(MEF90HookesLawIsotropic2D) :: MEF90HookesLawSumIso2D
