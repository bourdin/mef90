...
!  Geom%exoid = EXCRE (Geom%filename, EXCLOB, exo_cpu_ws, exo_io_ws,iErr)
  Params%Sim_Str   = Trim(EXO_Str) // '_four'
...
Contains
  Subroutine Write_F2D(Geom, Params, Node_db)
    ...
    Do iS = 1, Geom%Num_Nodes  
       If  ((Node_db(iS)%Coord%X*Node_db(iS)%Coord%Y) >= 0) Then
          Rhs_Result(iS)=1.0_Kr
       Else
          Rhs_Result(iS)=-1.0_Kr
       End If
       
    End Do
    Print*, ' Number of one :',     count(Rhs_Result==1.0)
    Print*, ' Number of none :',     count(Rhs_Result==-1.0)
    Print*, ' Number of zero :',     count(Rhs_Result==0.0)
  ...
  Subroutine Write_F3D(Geom, Params, Node_db)
    ...
    Rhs_Result=0.0_Kr 
    Do iS = 1, Geom%Num_Nodes  
       If  ((Node_db(iS)%Coord%X*Node_db(iS)%Coord%Y*Node_db(iS)%Coord%Z) >= 0) Then
          Rhs_Result(iS)=1.0_Kr
       Else
          Rhs_Result(iS)=-1.0_Kr
       End If
