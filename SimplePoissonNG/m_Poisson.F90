...
#define __FUNCT__ "SimplePoissonInit"
   Subroutine SimplePoissonInit(AppParam)
...
!      PetscBool                                    :: flg
      ...
      AppParam%verbose = 0
      Call PetscOptionsGetInt(PETSC_NULL_CHARACTER,'-verbose',AppParam%verbose,Flag,ierr);CHKERRQ(ierr)
      
      AppParam%Restart = PETSC_FALSE
      Call PetscOptionsGetBool(PETSC_NULL_CHARACTER,'-restart',AppParam%restart,Flag,ierr);CHKERRQ(ierr)
      
      AppParam%splitIO = PETSC_FALSE
      Call PetscOptionsGetBool(PETSC_NULL_CHARACTER,'-splitIO',AppParam%splitIO,Flag,ierr);CHKERRQ(ierr)
      
      Call PetscOptionsGetString(PETSC_NULL_CHARACTER,'-p',AppParam%prefix,HasPrefix,ierr);CHKERRQ(ierr)
      ...
      AppParam%TestCase = 1
      Call PetscOptionsGetInt(PETSC_NULL_CHARACTER,'-test',AppParam%TestCase,Flag,ierr);CHKERRQ(ierr)
      
      If (AppParam%verbose > 1) Then
         Write(filename,101) Trim(AppParam%prefix),MEF90_MyRank
         Call PetscViewerASCIIOpen(PETSC_COMM_SELF,filename,AppParam%MyLogViewer,ierr);CHKERRQ(ierr);  
