...
Contains
   ...
   Subroutine SimplePoissonInit(AppCtx)
      ...
      AppCtx%NumSteps = 1
      Call PetscOptionsGetInt(PETSC_NULL_CHARACTER, '-numstep',       AppCtx%NumSteps, Flag, iErr); CHKERRQ(iErr)
      AppCtx%tmin = 1.
      Call PetscOptionsGetReal(PETSC_NULL_CHARACTER, '-tmin',       AppCtx%tmin, Flag, iErr); CHKERRQ(iErr)
      AppCtx%tmax = 1.
      Call PetscOptionsGetReal(PETSC_NULL_CHARACTER, '-tmax',       AppCtx%tmax, Flag, iErr); CHKERRQ(iErr)
...
104 Format('Collective output redirected to file ', A, '\n')
   ...
   200 Format(A, '-', I4.4, '.gen')
      ...
      If (.NOT. AppCtx%AppParam%Restart) Then
