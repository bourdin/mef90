...
#include "finclude/petscmatdef.h"
   ...
   Call VecCreate(PETSC_COMM_WORLD, VVec, iErr); CHKERRQ(iErr)
   Call VecSetType(VVec, VECMPI, iErr); CHKERRQ(iErr)
   Call VecGetSize(VScal, nglob, iErr); CHKERRQ(iErr)
   Call VecGetLocalSize(VScal, nloc, iErr); CHKERRQ(iErr)
   Call VecSetSizes(VVec, nloc, nglob, iErr); CHKERRQ(iErr)
   ...
   Call SectionRealToVec(SScal, ScatterScal, SCATTER_FORWARD,  VVec, ierr); CHKERRQ(ierr)
   Call VecView(VVec, PETSC_VIEWER_STDOUT_WORLD, iErr); CHKERRQ(iErr)
...
!!!$ !      Val = i
!!!$ !      Call MeshUpdateClosure(MeshTopology%mesh, SScal, MeshTopology%Num_Elems+i-1, Val, iErr); CHKERRQ(iErr)
!!!$ !   End Do
