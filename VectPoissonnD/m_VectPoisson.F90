#if defined PB_2D
Module m_VectPoisson2D
#elif defined PB_3D
Module m_VectPoisson3D
#endif
...
#if defined PB_2D
      Type(Element2D), Dimension(:), Pointer       :: Elem
#elif defined PB_3D
      Type(Element3D), Dimension(:), Pointer       :: Elem
#endif
      Type(SectionReal)                            :: U
...
Contains
   ...
   Subroutine VectPoissonInit(AppCtx)
...
104 Format('Collective output redirected to file ', A, '\n')
      ...
      Call DMMeshGetVertexSectionReal(AppCtx%MeshTopology%mesh, 'U', AppCtx%MeshTopology%Num_Dim, AppCtx%U, iErr); CHKERRQ(iErr)
      Call DMMeshGetCellSectionReal(AppCtx%MeshTopology%mesh, 'GradU', AppCtx%MeshTopology%Num_Dim**2, AppCtx%GradU, iErr); CHKERRQ(iErr)
      Call DMMeshGetVertexSectionReal(AppCtx%MeshTopology%mesh, 'F', AppCtx%MeshTopology%Num_Dim, AppCtx%F, iErr); CHKERRQ(iErr)
      Call DMMeshCreateGlobalScatter(AppCtx%MeshTopology%mesh, AppCtx%U, AppCtx%Scatter, iErr); CHKERRQ(iErr)
      Call DMMeshCreateVector(AppCtx%MeshTopology%mesh, AppCtx%U, AppCtx%RHS, iErr); CHKERRQ(iErr)
     
      !!! Allocate and initialize the Section for the flag 
      Call DMMeshGetVertexSectionInt(AppCtx%MeshTopology%mesh, 'BCFlag', AppCtx%MeshTopology%Num_Dim, AppCtx%BCFlag, iErr); CHKERRQ(iErr)
   ...
   200 Format(A, '-', I4.4, '.gen')
      ...
      Else
         ...
         Call EXOFormat_VectPoisson(AppCtx)
         Call PetscLogStagePop(iErr); CHKERRQ(iErr)
   ...
   End Subroutine VectPoissonInit
   
!! Modified for the vectorial case
 Subroutine EXOFormat_VectPoisson(AppCtx)
      Type(AppCtx_Type)                            :: AppCtx
      PetscInt                                     :: iErr
...
#if defined PB_2D
      Call EXPVP (AppCtx%MyEXO%exoid, 'n', 4, iErr)
      Call EXPVAN(AppCtx%MyEXO%exoid, 'n', 4, (/'U_x', 'U_y', 'F_x', 'F_y'/), iErr)
      Call EXPVP (AppCtx%MyEXO%exoid, 'e', 4, iErr)
      Call EXPVAN(AppCtx%MyEXO%exoid, 'e', 4, (/'Grad U_XX', 'Grad U_XY', 'Grad U_YX', 'Grad U_YY'/), iErr)
#elif defined PB_3D
      Call EXPVP (AppCtx%MyEXO%exoid, 'n', 6, iErr)
      Call EXPVAN(AppCtx%MyEXO%exoid, 'n', 6, (/'U_X', 'U_Y', 'U_Z', 'F_X', 'F_Y', 'F_Z'/), iErr)
      Call EXPVP (AppCtx%MyEXO%exoid, 'e', 9, iErr)
      Call EXPVAN(AppCtx%MyEXO%exoid, 'e', 9, (/'Grad U_XX', 'Grad U_XY', 'Grad U_XZ', 'Grad U_YX', 'Grad U_YY', 'Grad U_YZ', 'Grad U_ZX', 'Grad U_ZY' ,'Grad U_ZZ'/), iErr)
#endif
      Call EXPTIM(AppCtx%MyEXO%exoid, 1, 1.0_Kr, iErr)

      Call EXCLOS(AppCtx%MyEXO%exoid, iErr)
   End Subroutine EXOFormat_VectPoisson
   ...
   Subroutine Solve(AppCtx)
      ...
      Call DMMeshCreateVector(AppCtx%MeshTopology%mesh, AppCtx%U, U_Vec, iErr); CHKERRQ(iErr)
...
100 Format('KSP converged in ', I5, ' iterations. KSPConvergedReason is ', I2, '\n')
   ...
   Subroutine RHSAssembly(AppCtx)
      ...
      Call DMMeshGetVertexSectionReal(AppCtx%MeshTopology%mesh, 'RHSSec', NumDoFPerVertex, RHSSec, iErr); CHKERRQ(iErr)
      Do_Elem_iBlk: Do iBlk = 1, AppCtx%MeshTopology%Num_Elem_Blks
         ...
         Do_Elem_iE: Do iELoc = 1, AppCtx%MeshTopology%Elem_Blk(iBlk)%Num_Elems
            ...
            Call DMMeshUpdateAddClosure(AppCtx%MeshTopology%Mesh, RHSSec, iE-1, RHSElem, iErr); CHKERRQ(iErr)
         End Do Do_Elem_iE
         DeAllocate(RHSElem)
      End Do Do_Elem_iBlk
   ...
   Subroutine RHSAssemblyLocal(iE, AppCtx, RHSElem)
     ...
     Type (Vect3D)                           :: TmpRHS    
      ...
      NumGauss = Size(AppCtx%Elem(iE)%BF,2)
      Allocate(BCFlag(NumDoF))
      Call DMMeshRestrictClosureInt(AppCtx%MeshTopology%mesh, AppCtx%BCFlag, iE-1, NumDoF, BCFlag, iErr); CHKERRQ(ierr)
      Allocate(F(NumDoF))
      Call DMMeshRestrictClosure(AppCtx%MeshTopology%mesh, AppCtx%F, iE-1, NumDoF, F, iErr); CHKERRQ(ierr)
      Do iGauss = 1, NumGauss
         TmpRHS = 0.0_Kr
         Do iDoF2 = 1, NumDoF
     ...
     Type(Vect3D)                                  :: F_Elem, U_Elem  
      ...
      Do_Elem_iBlk: Do iBlk = 1, AppCtx%MeshTopology%Num_Elem_Blks
         Do_Elem_iE: Do iELoc = 1, AppCtx%MeshTopology%Elem_Blk(iBlk)%Num_Elems
            ...
            Call DMMeshRestrictClosure(AppCtx%MeshTopology%mesh, AppCtx%F, iE-1, NumDoF, F, iErr); CHKERRQ(ierr)
            Allocate(U(NumDoF))
            Call DMMeshRestrictClosure(AppCtx%MeshTopology%mesh, AppCtx%U, iE-1, NumDoF, U, iErr); CHKERRQ(ierr)
            Do iGauss = 1, NumGauss
               Strain_Elem = 0.0_Kr
               Stress_Elem = 0.0_Kr
               F_Elem      = 0.0_Kr
               U_Elem      = 0.0_Kr
               Do iDoF = 1, NumDoF
                  Stress_Elem = Stress_Elem + AppCtx%Elem(iE)%Der_BF(iDoF, iGauss) * U(iDoF)
                  Strain_Elem = Strain_Elem + AppCtx%Elem(iE)%Der_BF(iDoF, iGauss) * U(iDoF)
...
#endif
            Call DMMeshUpdateClosure(AppCtx%MeshTopology%Mesh, AppCtx%GradU, iE-1, Grad_Ptr, iErr)
         End Do Do_Elem_iE
      End Do Do_Elem_iBlk
      DeAllocate(Grad_Ptr)
      Call PetscLogEventEnd  (AppCtx%LogInfo%PostProc_Event, iErr); CHKERRQ(iErr)
   ...
   Subroutine VectPoissonFinalize(AppCtx)   
      Type(AppCtx_Type)                            :: AppCtx
      ...
      Call MEF90_Finalize()
103 Format(A,'.log')
   End Subroutine VectPoissonFinalize

#if defined PB_2D
End Module m_VectPoisson2D
#elif defined PB_3D
End Module m_VectPoisson3D
#endif
