...
The reader is kindly invited to read the appendix A to see the perfect plasticity in the variational formulation.
...
Remarks: Von-Mises, Durcker-Prager are particular cases of Cap-Model similar with Tresca for Mohr-Coulomb.
...
$$ \mathcal{E}(u,\alpha,p) = \int_{\Omega} \Big[  \frac{1}{2} a(\alpha) \mathtt{A} (e(u)-p):(e(u)-p) + b(\alpha) \int_0^t \sup_{\varsigma \in K}  \{ \varsigma : \dot{p} \} \mathrm{d}\tau + \frac{k}{4c_w} \Big( \frac{w(\alpha)}{\ell} + \ell |\nabla \alpha |^2 \Big) \Big] \mathrm{d}x$$
...
\begin{algorithm}
    ...
    \label{algo:Ductile_Algo}
        ...
        \For{$i=0$ to $N$}
            \While{$\left| \alpha^j_i-\alpha^{j-1}_i\right|_{L^\infty} \le \delta_{\alpha}$}
...
\textcolor{red}{Unilateral contact models are being reimplemented and may not work at the moment}.

\subsection{Amor-Marigo-Maurini model (Hydrostatic deviatoric)}

Properties of such model can be found in \cite{Amor-Marigo-Maurini-2009}.
...
The evaluation of a crack volume in a domain is :
...
By plugging this crack pressure work into the total energy it turns out,
...
By letting “p" the crack pressure as a loading parameter the problem becomes not well posed, it is similar to the stress controlled body. The idea is to control the incompressible volume of fluid injected into cracks. If there is no \textit{in-situ} stresses applied on $\Omega$, the problem remains linear. The technique is to find the pressure $p_t$ where at the equilibrium is associated to the target volume $V_t$. For that, let applied any positive pressure for example $\bar{p}$ then calculate the volume of crack pressure for this $\bar{p}$ denoted $\bar{V}$. Knowing the volume target the pressure to applied is :
...
vDef can handle input and result files in two different ways: through the \verb+-prefix+ option or the \verb+-geometry+ \verb+-reult+ option pair.
