...
The reader is kindly invited  to read the appendix to better understand perfect plasticity.
...
Both perfect plasticity and gradient damage model are expressed in a variational formulation and have been presented previously, now let's coupled plasticity with damage using the function $b(\alpha)$ such as,

$$ \mathcal{E}(u,\alpha,p) = \int_{\Omega} \Big[  \frac{1}{2} a(\alpha) \mathtt{A} (e(u)-p):(e(u)-p) + b(\alpha) \int_0^t \sup_{\sigma \in K}  \{ \sigma : \dot{p} \} \mathrm{d}\tau + \frac{k}{4c_w} \Big( \frac{w(\alpha)}{\ell} + \ell |\nabla \alpha |^2 \Big) \Big] \mathrm{d}x$$

with $\sigma=a(\alpha)\mathtt{A} (e(u)-p)$. 

The algorithm is based on the decreasing total energy using alternate minimization with respect to $(u,\alpha,p)$. The minimization with respect to $(u,p)$ is respectively solving a linear problem for $u$, and for $p$, making projection onto the admissible yield surface on each element. Let plug this $(u,p)$ minimization into the $\alpha$ minimization, the algorithm takes the form Algo \ref{algo:Ductile_Algo}.
...
\begin{algorithm}
    ...
    \label{algo:Ductile_Algo}
        ...
        \For{$i=0$ to $N$}
            \While{$\left| \alpha^j-\alpha^{j-1}\right|_{L^\infty} \le \delta_{\alpha}$}
                \State{$\alpha^{j+1}_{i} \longleftarrow\underset{\alpha_{i-1}\leq\alpha\leq1}{\argmin} \; \mathcal{E}(u_i^j,\alpha,p_i^j)$}
                \While{$\left| p^{j+1,k}-p^{j+1,k-1}\right|_{L^\infty} \le \delta_{p}$}
...
$$W^{free}(u,\alpha)= \frac{K}{2} \tr^-({e(u)}) + a(\alpha) \Big( \frac{K}{2}\tr ^+({e(u)}) + \mu e_D(u):e_D(u) \Big) $$ where $K=\frac{(\lambda+2\mu)}{3}$ in 3D and $K=\frac{(\lambda+2\mu)}{2}$ in 2D.


\subsection{Masonry}

Masonry model try to do the same as Amor-Marigo-Maurini model but in the principal deformation basis such that 

$$W^{free}(u,\alpha)= \frac{1}{2} a(\alpha) \mathtt{A} e^+(u):e^+(u) + \frac{1}{2} \mathtt{A} \big( e(u)-e^+(u) \big): \big( e(u)-e^+(u) \big) $$

$e=e^+ + e^-$ , where $e$ is the principal strain. $e^+$ is the projection such as minimize the energy norm ,

$$|| e^+ - e ||_A = \min_{\varepsilon \in S} || \varepsilon - e||_A$$


Because the projection is not differentiable we do alternate projection until convergence of the displacement.

\section{Volume controlled and work controlled }


A variational formulation of a crack volume in a domain is :
...
\begin{itemize}
\item traction pre-stresses $\bar{\sigma_2} e_2 \otimes e_2$ applied on surfaces $(*, \pm l/2,*)$
...
What is the influence of the behaviors resfense


\item Use clamped boundary displacement
\end{enumerate}

\section{Burst experiment in 2D}
The idea of burst experiement is to calculate the stress intensity factor of rocks. The experiment consist in keeping the ratio beteween the injected internal pressure and the confining pressure constant. Then increase the internal pressure until the rock breaks.
...
The Sneddon problem is a penny shape crack of length $(2 l_0)$ included in a 2D plate, where a constant pressure is applied on crack. Under the assumption of an infinte plate, the solution of the problem is:
