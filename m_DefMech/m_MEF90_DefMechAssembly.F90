...
subroutine MEF90DefMechOperatorDisplacement(snesDisplacement, displacement, residual, MEF90DefMechCtx, ierr)
   ...
   if (.not. PetscObjectIsNull(setIS)) then
      ...
      do set = 1, size(setID)
         ...
         if (.not. PetscObjectIsNull(setPointIS)) then
            ...
            PetscCall(MEF90DefMechGetATModel(MEF90DefMechCtx%comm, prefix, dim, ATModel, ierr))
            PetscCall(ATModel%setFromOptions(ierr))
            PetscCall(MEF90DefMechGetSplit(MEF90DefMechCtx%comm, prefix, Split, ierr))
            PetscCall(Split%setFromOptions(ierr))
            PetscCall(MEF90GetHookesLaw(MEF90DefMechCtx%comm, prefix, MEF90_DIM, HookesLaw, ierr))
            ...
            do cell = 1, size(setPointID)
               ...
               do iGauss = 1, numGauss
                  ...
                  if (.not. ATModel%isElastic) then
                     ...
                     PetscCall(DMPlexVecRestoreClosure(dmDamage, PETSC_NULL_SECTION, damageLocal, setPointID(cell), PETSC_NULL_INTEGER, damageDof, ierr))
                  end if
   ...
   !! Adding terms in planestrain for plasticity with tr(p) = 0
               ...
               PetscCall(DMPlexVecSetClosure(dmDisplacement, PETSC_NULL_SECTION, residualLocal, setPointID(cell), residualDof, ADD_VALUES, ierr))
            end do ! cell
            ...
            !          PetscCall(DMPlexVecGetClosure(dmDamage,PETSC_NULL_SECTION,MEF90DefMechCtx%damageLocal(1),setPointID(cell),PETSC_NULL_INTEGER,damageDof,ierr))
            !          Do iDof = 1,numDofDamage
            !             GradientDamageGauss = GradientDamageGauss + damageDof(iDof) * elemScal(cell)%Grad_BF(iDof,iGauss)
            !          End Do ! iDof numDofDamage
            !          PetscCall(DMPlexVecRestoreClosure(dmDamage,PETSC_NULL_SECTION,MEF90DefMechCtx%damageLocal(1),setPointID(cell),PETSC_NULL_INTEGER,damageDof,ierr))
   ...
   PetscCall(DMLocalToGlobalBegin(dmDisplacement, residualLocal, ADD_VALUES, residual, ierr))
   PetscCall(DMLocalToGlobalEnd(dmDisplacement, residualLocal, ADD_VALUES, residual, ierr))
   PetscCall(DMRestoreLocalVector(dmDisplacement, residualLocal, ierr))
...
subroutine MEF90DefMechWork(MEF90DefMechCtx, bodyForceWork, boundaryForceWork, ierr)
   ...
   if (.not. PetscObjectIsNull(setIS)) then
      ...
      do set = 1, size(setID)
         ...
         PetscCallMPI(MPI_AllReduce(myWork, bodyForceWork(set), 1, MPIU_SCALAR, MPI_SUM, MEF90DefMechCtx%comm, ierr))
      end do ! set
      PetscCall(ISRestoreIndices(setIS, setID, ierr))
      PetscCall(VecRestoreArray(MEF90DefMechCtx%bodyForce, bodyForceArray, ierr))
      PetscCall(ISDestroy(setIS, ierr))
   ...
   if (.not. PetscObjectIsNull(setIS)) then
      ...
      do set = 1, size(setID)
         ...
         PetscCallMPI(MPI_AllReduce(myWork, boundaryForceWork(set), 1, MPIU_SCALAR, MPI_SUM, MEF90DefMechCtx%comm, ierr))
      end do ! set
      PetscCall(VecRestoreArray(MEF90DefMechCtx%boundaryForce, boundaryForceArray, ierr))
      PetscCall(VecRestoreArray(MEF90DefMechCtx%pressureForce, pressureForceArray, ierr))
      PetscCall(ISDestroy(setIS, ierr))
...
subroutine MEF90DefMechCohesiveEnergy(MEF90DefMechCtx, cohesiveEnergy, ierr)
   ...
   if (.not. PetscObjectIsNull(setIS)) then
      ...
      do set = 1, size(setID)
         ...
         PetscCallMPI(MPI_AllReduce(myCohesiveEnergy, cohesiveEnergy(set), 1, MPIU_SCALAR, MPI_SUM, MEF90DefMechCtx%comm, ierr))
      end do ! set
      PetscCall(ISRestoreIndices(setIS, setID, ierr))
      PetscCall(ISDestroy(setIS, ierr))
   end if ! setIS
...
! #endif
         ...
         PetscCallMPI(MPI_AllReduce(myEnergy, energy(set), 1, MPIU_SCALAR, MPI_SUM, MEF90DefMechCtx%comm, ierr))
      end do ! set
      PetscCall(ISRestoreIndices(setIS, setID, ierr))
      PetscCall(VecRestoreArray(MEF90DefMechCtx%plasticStrain, plasticStrainArray, ierr))
      PetscCall(ISDestroy(setIS, ierr))
...
subroutine MEF90DefMechOperatorDamage(snesDamage, damage, residual, MEF90DefMechCtx, ierr)
   ...
   !! Adding terms in planestrain for plasticity with tr(p) = 0
               ...
               PetscCall(DMPlexVecRestoreClosure(dmDisplacement, PETSC_NULL_SECTION, MEF90DefMechCtx%displacementLocal, setPointID(cell), PETSC_NULL_INTEGER, displacementDof, ierr))
               PetscCall(DMPlexVecRestoreClosure(dmDamage, PETSC_NULL_SECTION, damageLocal, setPointID(cell), PETSC_NULL_INTEGER, damageDof, ierr))
               PetscCall(DMPlexVecSetClosure(dmDamage, PETSC_NULL_SECTION, residualLoc, setPointID(cell), residualDof, ADD_VALUES, ierr))
            end do ! cellresidualLoc
   ...
   PetscCall(DMLocalToGlobalBegin(dmDamage, residualLoc, ADD_VALUES, residual, ierr))
   PetscCall(DMLocalToGlobalEnd(dmDamage, residualLoc, ADD_VALUES, residual, ierr))
   PetscCall(DMRestoreLocalVector(dmDamage, residualLoc, ierr))
...
subroutine MEF90DefMechTAOGradientDamage(taoDamage, damage, residual, MEF90DefMechCtx, ierr)
   ...
   ! PetscCall(DMGlobalToLocal(dmDamage, damage, INSERT_VALUES, MEF90DefMechCtx%damageLocal(1), ierr))
...
subroutine MEF90DefMechCrackVolume(MEF90DefMechCtx, CrackVolume, ierr)
   ...
   if (.not. PetscObjectIsNull(setIS)) then
      ...
      do set = 1, size(setID)
         ...
         PetscCallMPI(MPI_AllReduce(myCrackVolume, CrackVolume(set), 1, MPIU_SCALAR, MPI_SUM, MEF90DefMechCtx%comm, ierr))
      end do ! set
      PetscCall(ISRestoreIndices(setIS, setID, ierr))
      PetscCall(ISDestroy(setIS, ierr))
   end if ! setIS
