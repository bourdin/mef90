...
!!!
   ...
   Subroutine MEF90DefMechTAOObjectiveDamage(taoDamage,damage,energy,MEF90DefMechCtx,ierr)
      ...
      Type(tDM)                                          :: dmDamage,dmDisplacement,dmTemperature,dmPlasticStrain
      Type(tDM)                                          :: dmBodyForce,dmBoundaryForce,dmPressureForce
      Type(tPetscSection)                                :: sectionPlasticStrain,sectionBodyForce,sectionBoundaryForce,sectionPressureForce
      PetscReal,Dimension(:),Pointer                     :: damageDof,displacementDof,temperatureDof
      PetscReal,Dimension(:),Pointer                     :: plasticStrainArray,bodyForceArray,boundaryForceArray,pressureForceArray
      PetscInt                                           :: vecOffset
      Type(MEF90_MATS)                                   :: plasticStrainCell
      Type(tIS)                                          :: setIS,setPointIS
      PetscInt,dimension(:),Pointer                      :: setID,setPointID
      ...
      Type(MEF90_ELEMENT_SCAL),Dimension(:),Pointer      :: elemScal
      DMPolytopeType                                     :: cellGeometry
      Type(MEF90ElementType)                             :: elemScalType,elemVectType
      ...
      Type(MEF90_VECT)                                   :: gradDamageGauss
      Type(MEF90_VECT)                                   :: bodyForce,boundaryForce,pressureForce
      PetscReal,Dimension(:),Allocatable                 :: energySet
      PetscReal                                          :: damageGauss,temperatureGauss,EEDPlus,EEDMinus,elasticEnergyDensityGauss,C1,myEnergy
      ...
      If (setIS /= PETSC_NULL_IS) Then
         ...
         Do set = 1,size(setID)
            ...
            If (setPointIS /= PETSC_NULL_IS) Then
               ...
               PetscCall(ISGetIndicesF90(setPointIS,setPointID,ierr))
               PetscCall(DMPlexGetCellType(dmDamage,setPointID(1),cellGeometry,ierr))
               PetscCall(MEF90ElementGetType(MEF90CtxGlobalOptions%elementFamily,MEF90CtxGlobalOptions%elementOrder,cellGeometry,elemVectType,ierr))
               PetscCall(MEF90ElementGetType(MEF90CtxGlobalOptions%elementFamily,MEF90CtxGlobalOptions%elementOrder,cellGeometry,elemScalType,ierr))
...
! #endif
               ...
               If (norm2(cellSetOptions%bodyForce) /= 0.0_Kr) Then
                  Do cell = 1,size(setPointID)
                     ...
                     Do iGauss = 1,numGauss
                        Do iDof = 1,numDofDisplacement
                           myEnergy = myEnergy - elemVect(cell)%Gauss_C(iGauss) * displacementDof(iDof) * (bodyForce .DotP. elemVect(cell)%BF(iDof,iGauss))
                        End Do ! iDof numDofDisplacement
                     End Do ! iGauss
                     PetscCall(DMPlexVecRestoreClosure(dmDisplacement,PETSC_NULL_SECTION,MEF90DefMechCtx%displacementLocal,setPointID(cell),displacementDof,ierr))
                  End Do ! cell
               ...
               !       Do iGauss = 1,numGauss
               !          Do iDof = 1,numDofDisplacement
               !             myEnergy = myEnergy + elemVect(cell)%Gauss_C(iGauss) * matpropSet%cohesiveStiffness * (U0Gauss .DotP. U0Gauss)
               !          End Do ! iDof numDofDisplacement
               !       End Do ! iGauss
               !    End Do ! cell
               ! End If ! cohesiveDisplacement
               ...
               !          End Do ! iDof numDofDamage
               !          Do iDof = 1,numDofDisplacement
               !             myEnergy = myEnergy + elemVect(cell)%Gauss_C(iGauss) * &
               !                                  CrackPressureCell * displacementDof(iDof) * (GradientDamageGauss .DotP. elemVect(cell)%BF(iDof,iGauss))
               !          End Do ! iDof numDofDisplacement
               !       End Do ! iGauss
               !       PetscCall(DMPlexVecRestoreClosure(dmDisplacement,PETSC_NULL_SECTION,MEF90DefMechCtx%displacementLocal,setPointID(cell),displacementDof,ierr))
               !       PetscCall(DMPlexVecRestoreClosure(dmDamage,PETSC_NULL_SECTION,MEF90DefMechCtx%damageLocal,setPointID(cell),damageDof,ierr))
            ...
            End If ! setPointIS
            PetscCall(ISDestroy(setPointIS,ierr))
            PetscCallMPI(MPI_AllReduce(myEnergy,energySet(set),1,MPIU_SCALAR,MPI_SUM,MEF90DefMechCtx%MEF90Ctx%comm,ierr))
            energy = energy + energySet(set)
         End Do ! set
         PetscCall(ISRestoreIndicesF90(setIS,setID,ierr))
         PetscCall(VecRestoreArrayF90(MEF90DefMechCtx%plasticStrain,plasticStrainArray,ierr))
         DeAllocate(energySet)
      End If ! setIS
      PetscCall(ISDestroy(setIS,ierr))
            ...
            If (setPointIS /= PETSC_NULL_IS) Then
               ...
               If (norm2(faceSetOptions%boundaryForce) /= 0.0_Kr) Then
                  Do cell = 1,size(setPointID)
                     ...
                     Do iGauss = 1,numGauss
                        Do iDof = 1,numDofDisplacement
                           myEnergy = myEnergy - elemVect(cell)%Gauss_C(iGauss) * displacementDof(iDof) * (boundaryForce .DotP. elemVect(cell)%BF(iDof,iGauss))
                        End Do ! iDof numDofDisplacement
                     End Do ! iGauss
                     PetscCall(DMPlexVecRestoreClosure(dmDisplacement,PETSC_NULL_SECTION,MEF90DefMechCtx%displacementLocal,setPointID(cell),displacementDof,ierr))
                  End Do ! cell
               ...
               If (faceSetOptions%pressureForce /= 0.0_Kr) Then
                  Do cell = 1,size(setPointID)
                     ...
                     Do iGauss = 1,numGauss
                        Do iDof = 1,numDofDisplacement
                           myEnergy = myEnergy - elemVect(cell)%Gauss_C(iGauss) * displacementDof(iDof) * (pressureForce .DotP. elemVect(cell)%BF(iDof,iGauss))
                        End Do ! iDof numDofDisplacement
                     End Do ! iGauss
                     PetscCall(DMPlexVecRestoreClosure(dmDisplacement,PETSC_NULL_SECTION,MEF90DefMechCtx%displacementLocal,setPointID(cell),displacementDof,ierr))
                  End Do ! cell
