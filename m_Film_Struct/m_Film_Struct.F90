...
#include "finclude/petscdef.h"
 ...
 Contains
   ...
   Subroutine EXOProperty_InitBCWFlag(dBCFlag, dEXO, dMeshTopology)
      ...
      Do i = 1, dMeshTopology%Num_Node_Sets
         Allocate(iFlag(1))
         iFlag = dEXO%NSProperty( VarFrac_NSProp_BCWType )%Value( dMeshTopology%Node_Set(i)%ID )

         Do j = 1, dMeshTopology%Node_Set(i)%Num_Nodes
            Call SectionIntUpdate(dBCFlag%Component_Sec(1), dMeshTopology%Node_Set(i)%Node_ID(j) + dMeshTopology%Num_Elems-1, iFlag, ADD_VALUES, iErr); CHKERRQ(iErr)
         End Do
         DeAllocate(iFlag)
      End Do
   ...
   Subroutine MatProp2D_Write(MeshTopology, MatProp, filename)
      ...
      Do iBlk = 1, Size(MatProp)
         Write(F_OUT,120) iBlk, MatProp(iBlk)%FracToughness, MatProp(iBlk)%DelamToughness, MatProp(iBlk)%Ksubst, MatProp(iBlk)%Hookes_Law, MatProp(iBlk)%Therm_Exp
      End Do
      Close(F_OUT)
...
!!! The default is now to save element variables
   ...
   Subroutine GenHL_Cubic2D_LambdaMu(lambda, mu1, mu2, theta, A)
      ...
      DeAllocate(R)
   End Subroutine GenHL_Cubic2D_LambdaMu
   
   Subroutine GenHL_Cubic2DPlaneStress_Zener(B, C, Cp, theta, A)
   ...
   Subroutine GenHL_Cubic2DPlaneStress_Voigt(C11, C12, C44, theta, A)
      PetscReal, Intent(IN)               :: C11, C12, C44, theta
      Type(Tens4OS2D)                     :: TmpTens
      Type(Tens4OS2D), Intent(OUT)        :: A
      PetscReal, Dimension(:,:), Pointer  :: R
   ...
   Subroutine GenHL_Ortho2D_EGnu(E, G, nu, theta, A)
   ...
   Subroutine GenHL_Ortho2D_EGmu(E, G, mu, theta, A)
      PetscReal, Intent(IN)               :: E, G, mu, theta
      Type(Tens4OS2D), Intent(OUT)        :: A
      
      PetscReal                           :: nu, lambda, mu1, mu2
