...
if options.meshsize == None:
  parser.error("must specify mesh size with -h or --meshsize")
...
scheme2d   = DCBconfig.get('mesh', 'scheme2d')
...
### mesh
###
cubit.cmd("surface with z_min>0 size %f" % float(options.meshsize))
cubit.cmd("surface with z_min>0 scheme %s" % scheme2d)
cubit.cmd("mesh surface with z_min>0")
###
### BC
###
cubit.cmd('nodeset 1 surface with z_min>0 in volume in GRIP1_3D')
cubit.cmd("nodeset 2 surface with z_min>0 in volume in GRIP2_3D")
###
cubit.cmd("block 1 surface with z_min>0 in volume in GRIP1_3D")
cubit.cmd("block 2 surface with z_min>0 in volume in GRIP2_3D")
cubit.cmd("block 3 surface with z_min>0 in volume in DCB_3D")
cubit.cmd("block 4 surface with z_min>0 in volume in LAYER1_3D")
cubit.cmd("block 5 surface with z_min>0 in volume in LAYER2_3D")
###
### export mesh
###
cubit.cmd('export mesh "%s" dimension 2 overwrite' % options.genfile)